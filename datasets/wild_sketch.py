...
class WildSketch(BaseImageDataset):
    """

    """
    dataset_dir_r = 'WildSketch'

    def __init__(self, root='', verbose=True, pid_begin=0, **kwargs):
        super(WildSketch, self).__init__()
        self.dataset_dir_r = osp.join(root, self.dataset_dir_r)
        self.train_dir_photo_r = osp.join(self.dataset_dir_r, 'train', 'images')
        self.train_dir_sketch_r = osp.join(self.dataset_dir_r, 'train', 'sketches')
        self.query_dir = osp.join(self.dataset_dir_r, 'test', 'sketches')
        self.gallery_dir = osp.join(self.dataset_dir_r, 'test', 'images')
    ...
    def _process_dir_query(self, dir_path, relabel=False):
        ...
        for img_path in sorted(img_paths):
            pid = int(osp.basename(img_path)[:-4])
            if pid == -1: continue  # junk images are just ignored
            assert 0 <= pid <= 400  # pid == 0 means background
