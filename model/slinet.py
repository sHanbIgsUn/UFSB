...
from model.clip.prompt_learner_domain import cfgc, load_clip_to_cpu_domain, TextEncoder_domain, PromptLearner_domain
...
class SliNet(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.cfg = cfgc()
        clip_model = load_clip_to_cpu_domain(self.cfg)
        self.clip_model = clip_model

        self.image_encoder = clip_model.visual
        self.text_encoder = TextEncoder_domain(clip_model)
        ...
        if args["dataset"] == "cddb":
            self.classifier_pool = nn.ModuleList([
                PromptLearner_domain(self.cfg, list(cddb_classnames.values()), self.clip_model)
                for i in range(args["total_sessions"])
            ])
            self.class_num = 2
        elif args["dataset"] == "domainnet":
            self.classifier_pool = nn.ModuleList([
                PromptLearner_domain(self.cfg, list(domainnet_classnames.values()), self.clip_model)
                for i in range(args["total_sessions"])
            ])
            self.class_num = 345
        elif args["dataset"] == "core50":
            self.classifier_pool = nn.ModuleList([
                PromptLearner_domain(self.cfg, list(core50_classnames.values()), self.clip_model)
                for i in range(args["total_sessions"])
            ])
            self.class_num = 50
        else:
