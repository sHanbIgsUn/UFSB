...
def do_train_stage2_ufsb(cfg,
                    ...
                    writer,
                    task=999,
                    loss_funces=None,sketch_list=None,prev_text_features=None,iter_start=0):
    ...
    device = "cuda"
    if task == 0 :
        epochs = cfg.SOLVER.STAGE2.MAX_EPOCHS_1
        epoch_start = 0
    else:
        epochs = cfg.SOLVER.STAGE2.MAX_EPOCHS_N
        epoch_start = cfg.SOLVER.STAGE2.MAX_EPOCHS_1 + (task - 1) * cfg.SOLVER.STAGE2.MAX_EPOCHS_N
    ...
    if device:
        ...
        if torch.cuda.device_count() > 1:
            ...
            num_classes = model.module.num_classes_list[task]
        else:
            # num_classes = model.num_classes[task]
            num_classes = model.num_classes_list[task]
    ...
    for epoch in range(epoch_start, epoch_start + epochs):
        ...
        for n_iter, (img, vid, target_cam, target_view) in enumerate(train_loader_stage2):
            ...
            with amp.autocast(enabled=True):
                ...
                loss = loss_fn(score, feat, target, target_cam, logits,writer,iter=sum_iter,sketch_cam=sketch_list[task])
        ...
        if task > 0 and 'bank' in cfg.MODEL.MODULE:
            ...
            for prev_task in range(task):
                ...
                # 提取历史任务数据
                images_p = torch.stack([image for tuple_list in model.data_bank['photo'].values() 
                                            for image, _, _ in tuple_list])
                images_s = torch.stack([image for tuple_list in model.data_bank['sketch'].values() 
                                            for image, _, _ in tuple_list])
                labels = torch.tensor(list(model.data_bank['sketch'].keys()))
                cams_p = torch.tensor([cam for tuple_list in model.data_bank['photo'].values() 
                                            for _, cam, _ in tuple_list])
                cams_s = torch.tensor([cam for tuple_list in model.data_bank['sketch'].values() 
                                            for _, cam, _ in tuple_list])

                keys_order_p = list(model.data_bank['photo'].keys())
                keys_order_s = list(model.data_bank['sketch'].keys())
                ...
                for batch_idx in range(0, len(labels), batch_size):
                    ...
                    with amp.autocast(enabled=True):
                        ...
                        loss = loss_funces[prev_task](score, feat, batch_labels, batch_cams, logits,None,0,sketch_list[prev_task])
